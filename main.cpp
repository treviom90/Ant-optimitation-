...
#include <iostream>
#include <string> // Para trabajar fácil com strings
#include <sstream> // Para trabajar fácil con strings
#include <math.h> // Para ayudar en calculos matemáticos
...
int main(int argc, char *argv[]) {
	...
	srand(time(0));
	// Inicializar el contador de interaciones
	int iteration = 0;

	// Configuración de las concentraciones iniciales de feromonas
	seed_initial_pheromone(false);
	// Tomando la distancia más larga más 1
	...
	//cin.get(); // espere un nuevo carácter y luego finalice la aplicación
	return 0;
}
...
void init_ant(int index) {
	...
	// Posicionando la hormiga en una ciudad aleatoria
...
void build_solution() {
	...
	for (int i = 0; i < POPULATION_SIZE; i++) {
		...
		// Hasta que pase todas las ciudades
		while (ants[i].position < (CITY_AMOUNT - 1)) {
			...
			// Agregar índices de feromonas y heurística
			for (int j = 0; j < CITY_AMOUNT; j++) {
				// Si la ciudad ya fue visitada, no ingrese al análisis
			...
			// Calculando la probabilidad de transición
			for (int j = 0; j < CITY_AMOUNT; j++) {
				// Si la ciudad ya fue visitada, no ingrese al análisis
				if (check_visit(i, j) == false) {
					if (pheromone_links[position][j] >= 0
...
void print_route(int ant_index, int route[CITY_AMOUNT], double distance) {
	string temp = "Ruta de las hormigas " + number_to_String(ant_index) + " : ";
	...
	temp += ". Tamaño: " + number_to_String(distance) + "\n";
	cout << temp << endl;
}

void print_pheromone() {
	string temp = "Tasas de feromonas:";
	temp += "\n{";
	for (int i = 0; i < CITY_AMOUNT; i++) {
		temp += "{";
		for (int j = 0; j < CITY_AMOUNT; j++) {
...
void print_result() {
	...
	cout << "Desviacion estandar:" << standard_deviation << endl;
}
